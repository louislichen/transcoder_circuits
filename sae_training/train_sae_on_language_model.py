...
def train_sae_on_language_model(
    ...
    while n_training_tokens < total_training_tokens:
            ...
            if sparse_autoencoder.cfg.use_tqdm:
                if sparse_autoencoder.cfg.is_sparse_connection:
                    pbar.set_description(
                        f"{n_training_steps}| MSE Loss {mse_loss.item():.6f} | L1 {l1_loss.item():.6f} | SCST {spacon_loss.item():.3f}"
                    )
                else:
                    pbar.set_description(
                        f"{n_training_steps}| MSE Loss {mse_loss.item():.6f} | L1 {l1_loss.item():.6f}"
                    )
        ...
        # Early stopping based on loss not improving
        ...
        if no_improve_steps >= early_stop_patience:
            print(f"[Early Stop] Stop at step {n_training_steps}, best loss={best_loss:.6f}")
            break
